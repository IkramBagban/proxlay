...
const WorkspaceMembers = () => {
  ...
  const invitedMembers = members?.filter(member => member.status?.toLowerCase() === 'invited') || [];
  const pendingMembers = members?.filter(member => member.status?.toLowerCase() === 'pending') || [];
    ...
    return (
      <tr key={member.id || index} className="hover:bg-gray-50 transition-colors">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-offset-1 ring-gray-100">
                <img
                  src={imageUrl || `https://ui-avatars.com/api/?name=${firstName}+${lastName}&background=random`}
        ...
        <td className="px-6 py-4">
          <Badge 
            variant="outline" 
            className={`capitalize font-medium ${getStatusColor(member.status)}`}
          >
            {member.status}
          </Badge>
        </td>
        ...
        <td className="px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'invites' && (
              <>
                <Button size="sm" variant="outline" className="hover:bg-green-50 hover:border-green-200 text-green-700">
                  Resend
                </Button>
                <Button size="sm" variant="outline" className="hover:bg-red-50 hover:border-red-200 text-red-700">
                  Cancel
                </Button>
              </>
            )}
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {activeTab === 'members' ? 'Joined' : activeTab === 'invites' ? 'Invited' : 'Requested'}
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Actions
                    </th>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">Invite more members</h3>
